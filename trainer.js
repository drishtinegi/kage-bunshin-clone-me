...
// data collection
// ----------------------
let samples = { clone_sign: [], erase_sign: [], not_sign: [] };
let recording = null;
let model = null;
...
function captureFrame(right, left) {
  ...
  document.getElementById("count-erase").textContent = samples.erase_sign.length;
...
function startCountdown(label) {
  ...
  const labelName =
    label === "clone_sign" ? "clone sign" :
    label === "erase_sign" ? "erase sign" :
    "other";

  statusEl.textContent = `Recording "${labelName}" in ${remaining}s — get into position!`;
...
function startRec(label) {
  ...
  statusEl.textContent = `Recording "${labelName}" — hold your pose!`;
...
["btn-rec-clone", "btn-rec-erase", "btn-rec-other"].forEach(id => {
  let label;
  if (id.includes("clone")) label = "clone_sign";
  else if (id.includes("erase")) label = "erase_sign";
  else label = "not_sign";
...
document.getElementById("btn-train").addEventListener("click", async () => {
  ...
  const nE = samples.erase_sign.length;

  if (nP < 100 || nE < 100 || nN < 100) {
    statusEl.textContent = "Need at least 20 samples per class.";
    return;
  }
  ...
  const xs = [], ys = [];
  const labelMap = { clone_sign: 0, erase_sign: 1, not_sign: 2 };

  Object.entries(samples).forEach(([label, arr]) => {
  arr.forEach(s => {
    xs.push(s);
    ys.push(labelMap[label]);
  });
  });
  ...
  const yT = tf.oneHot(tf.tensor1d(ys, "int32"), 3);
  ...
  statusEl.textContent = `Done! ${nP + nN + nE} samples. Model is live — test your sign above.`;
});
...
document.getElementById("import-data-input").addEventListener("change", e => {
  ...
  reader.onload = ev => {
    ...
    samples.erase_sign.push(...(data.erase_sign || []));
...
document.getElementById("btn-clear-data").addEventListener("click", () => {
  samples = { clone_sign: [], erase_sign: [], not_sign: [] };
